...
type
  TNotaFrm = class(TForm)
    ...
    procedure QMasterAfterOpen(DataSet: TDataSet);
  private
...
begin
  if QMasterAGEN.AsFloat=1 then LSuplier.Caption:='PT. WASTRA KARYA NUSANTARA' else LSuplier.Caption:='PT. KABANA TEXTILE';
...
' and substr(jns_order, 11, 3) in (:jns,:jns1) order by no_nota'; //and jns_order in (:jns1)';
...
QTotal_Rekap.SQL.Text:='select mu, sum(qty7)as qty7, sum(dpp) as dpp, sum(ppn) as ppn, sum(akhir) as akhir, '+
'sum(dpp_idr) as dpp_idr, sum(ppn_idr) as ppn_idr, sum(akhir_idr) as akhir_idr '+
'from ipisma_db2.vrekap_penjualan where tgl>=:pawal and tgl<=:pakhir and substr(jns_order, 11, 3) in (:jns,:jns1) group by mu';
...
' and substr(jns_order, 11, 3) in (:jns) order by no_nota';
QRekap_penj.DeclareVariable('pawal', otDate);
QRekap_penj.DeclareVariable('pakhir', otDate);
QRekap_penj.DeclareVariable('jns', otString);
QRekap_penj.SetVariable('pawal', VTglAwal1.Date);
...
QTotal_Rekap.SQL.Text:='select mu, sum(qty7)as qty7, sum(dpp) as dpp, sum(ppn) as ppn, sum(akhir) as akhir, '+
'sum(dpp_idr) as dpp_idr, sum(ppn_idr) as ppn_idr, sum(akhir_idr) as akhir_idr '+
'from ipisma_db2.vrekap_penjualan where trunc(tgl)>=:pawal and trunc(tgl)<=:pakhir and substr(jns_order, 11, 3) in (:jns) group by mu';
QTotal_Rekap.SetVariable('pawal', VTglAwal1.Date);
QTotal_Rekap.SetVariable('pakhir', VTglAkhir1.Date);
QTotal_Rekap.SetVariable('jns',vjenis);
QTotal_Rekap.Open;
...
QTotal_Rekap.SQL.Text:='select mu, sum(qty7)as qty7, sum(dpp) as dpp, sum(ppn) as ppn, sum(akhir) as akhir, '+
'sum(dpp_idr) as dpp_idr, sum(ppn_idr) as ppn_idr, sum(akhir_idr) as akhir_idr '+
'from ipisma_db2.vrekap_penjualan where trunc(tgl)>=:pawal and trunc(tgl)<=:pakhir group by mu';
QTotal_Rekap.SetVariable('pawal', VTglAwal1.Date);
QTotal_Rekap.SetVariable('pakhir', VTglAkhir1.Date);
QTotal_Rekap.Open;
wwDBGrid3.ColumnByName('QTY7').FooterValue:=FormatFloat('#,##0.00;(#,##0.00)',QTotal_Rekap.FieldByName('QTY7').AsFloat);
...
procedure TNotaFrm.QMasterAfterOpen(DataSet: TDataSet);
begin
  if QMasterAGEN.AsFloat=1 then LSuplier.Caption:='PT. WASTRA KARYA NUSANTARA' else LSuplier.Caption:='PT. KABANA TEXTILE';
end;
